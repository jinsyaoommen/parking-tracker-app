export const colors = {
  navy: {
    light: '#506973',
    medium: '#40545c',
    dark: '#303f45'
  },
  apricot: {
    light: '#ee9065',
    medium: '#be7350',
    dark: '#8e563c'
  },
  osloGray: {
    light: '#8b8c90',
    medium: '#6f7073',
    dark: '#535456'
  },
  valencia: {
    light: '#d44829',
    medium: '#a93920',
    dark: '#7f2b18'
  },
  cascadeBlue: {
    light: '#56a7e1',
    medium: '#4485b4',
    dark: '#336487'
  },
  bayleaf: {
    light: '#7eb98d',
    medium: '#649470',
    dark: '#4b6f54'
  },
  cascadeGreen: {
    light: '#58a618',
    medium: '#468413',
    dark: '#34630e'
  },
  black: {
    light: '#333333',
    medium: '#000000',
    dark: '#000000'
  },
  white: {
    light: '#ffffff',
    medium: '#ffffff',
    dark: '#ffffff'
  },
  riverbed: {
    light: '#425760',
    medium: '#34454c',
    dark: '#273439'
  },
  athensGray: {
    light: '#edeff3',
    medium: '#bdbfc3',
    dark: '#8e8f91'
  },
  silverSand: {
    light: '#efefed',
    medium: '#bfbfbd',
    dark: '#8f8f8f'
  },
  whiteSmoke: {
    light: '#f5f5f5',
    medium: '#D3D3D3',
    dark: '#939393'
  },
  poloBlue: {
    light: '#91aad2',
    medium: '#7488a8',
    dark: '#57667e'
  },
  shakespeare: {
    light: '#6aa5b7',
    medium: '#548492',
    dark: '#3f636d'
  },
  carrotOrange: {
    light: '#f89a1c',
    medium: '#c67b16',
    dark: '#955d10'
  },
  amber: {
    light: '#ffbf00',
    medium: '#cc9800',
    dark: '#997200'
  }
};

export const semanticColors = {
  default: {
    standard: colors.osloGray.light,
    hover: colors.osloGray.medium,
    active: colors.osloGray.dark
  },
  primary: {
    standard: colors.apricot.light,
    hover: colors.apricot.medium,
    active: colors.apricot.dark
  },
  info: {
    standard: colors.navy.light,
    hover: colors.navy.medium,
    active: colors.navy.dark
  },
  danger: {
    standard: colors.valencia.light,
    hover: colors.valencia.medium,
    active: colors.valencia.dark
  },
  warning: {
    standard: colors.amber.light,
    hover: colors.amber.medium,
    active: colors.amber.dark
  },
  secondary: {
    standard: colors.shakespeare.light,
    hover: colors.shakespeare.medium,
    active: colors.shakespeare.dark
  },
  success: {
    standard: colors.bayleaf.light,
    hover: colors.bayleaf.medium,
    active: colors.bayleaf.dark
  }
};
